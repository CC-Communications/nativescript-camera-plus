 ...
 */
class QBImagePickerControllerDelegateImpl extends NSObject
  ...
  public initWithCallbackAndOptions(
    ...
    if (options) {
      ...
      this._keepAspectRatio = types.isNullOrUndefined(options.keepAspectRatio)
        ? true
...
export class SwiftyDelegate extends NSObject {
  ...
  swiftyCamDidFinishProcessVideoAt(
    swiftyCam: SwiftyCamViewController,
    url: NSURL
  ) {
  ...
  swiftyCamDidFinishRecordingVideo(
    swiftyCam: SwiftyCamViewController,
    camera: CameraSelection
  ) {
...
export class MySwifty extends SwiftyCamViewController {
  ...
  private _imagePickerController: any;
  ...
  public static initWithOwner(
    ...
    // set default camera
    ctrl.defaultCamera =
      defaultCamera === "rear" ? CameraSelection.Rear : CameraSelection.Front;
  ...
  public closePicker() {
    this._imagePickerController.dismissViewControllerAnimatedCompletion(
      true,
      () => {
  ...
  public resize(width?: any, height?: any) {
    ...
    CLog(
      "view.bounds:",
      this.view.bounds.size.width + "x" + this.view.bounds.size.height
    );
    if (!this._resized) {
      this._resized = true;
      this._addButtons();
  ...
  public recordingReady(path: string) {
    if (
      (this._videoOptions && this._videoOptions.saveToGallery) ||
      this._owner.get().saveToGallery
    ) {
  ...
  public videoDidFinishSavingWithErrorContextInfo(
    path: string,
    error: NSError,
    contextInfo: any
  ) {
  ...
  public tookPhoto(photo: UIImage) {
    ...
    if (this._snapPicOptions && this._snapPicOptions.confirm) {
      ...
      this._imageConfirmBg = UIView.alloc().initWithFrame(
  ...
  public savePhoto() {
    if (this._photoToSave) {
      if (
        (this._snapPicOptions && this._snapPicOptions.saveToGallery) ||
        this._owner.get().saveToGallery
      ) {
      ...
      this._owner
        .get()
        .sendEvent(
          CameraPlus.photoCapturedEvent,
          new ImageAsset(this._photoToSave)
        );
      this.resetPreview();
    }
  }
  ...
  public chooseFromLibrary(options?: IChooseOptions): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      if (options) {
        ...
        keepAspectRatio = types.isNullOrUndefined(options.keepAspectRatio)
          ? true
          : options.keepAspectRatio;
      }
...
export class CameraPlus extends CameraPlusBase {
  ...
  constructor() {
    ...
    this._swifty = MySwifty.initWithOwner(
      new WeakRef(this),
      CameraPlus.enableVideo,
      CameraPlus.defaultCamera
    );
